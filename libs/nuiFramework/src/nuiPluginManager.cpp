...
nuiPluginFrameworkErrorCode::err nuiPluginManager::loadDefaultConfiguration()
{
	...
	std::ifstream settingsFile(manager.getRelativeToStartupPath(std::string("data/default_config.json")));
	Json::Value root;
	Json::Reader reader;
	...
	{
		std::string path = manager.getRelativeToStartupPath((*i).get("path", NULL).asString());
